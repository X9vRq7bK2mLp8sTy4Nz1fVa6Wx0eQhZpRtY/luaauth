...
(async function(){
  ...
  clearBtn.addEventListener('click', ()=> {
    codeEl.value = '';
    titleEl.value = '';
    out.textContent = 'paste obfuscated code and press create. loader link will appear here.';
  ...
  createBtn.addEventListener('click', async () => {
    ...
    if (!obfCode) { out.textContent = 'paste obfuscated payload'; return; }
    out.textContent = 'creating...';
    try {
      const res = await fetch('/api/create', {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'x-session-token': sessionToken },
        body: JSON.stringify({ title: titleEl.value || '', obfCode })
      });
      const j = await res.json();
      if (!res.ok) { out.textContent = 'error: ' + (j.error || JSON.stringify(j)); return; }
      const loader = j.rawUrl;
      loaderLink.href = loader;
      loaderLink.textContent = loader;
      meta.style.display = 'flex';
      out.textContent = 'created successfully. give loader url only to executor environments.';
    } catch (e) {
